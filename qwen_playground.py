...
device = "cuda"
model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3.5-9B").to(device)
...
def _generate_until(prompt_text: str, stop_strings, max_new_tokens: int, temperature: float | None = None) -> str:
    ...
    stopper = StopOnStrings(stop_strings, tokenizer, ids.shape[1])
...
def generate_turn_unconstrained(prompt_text: str):
    ...
    temp = None
    text = _generate_until(prompt_text, [ACTION_CLOSE], 8194, temp)
    parse_text = "<think>" + text
    print(parse_text)
    thought = m.group(1).strip() if (m := THINK_RE.search(parse_text)) else ""
    action = m.group(1).strip() if (m := ACTION_RE.search(parse_text)) else ""
...
You are an expert agent operating in the ALFRED Embodied Environment.<|im_end|>
<|im_start|>user
## Skill Knowledge
...
Your current observation is: You are in the middle of a room. Looking quickly around you, you see a armchair 1, a cabinet 4, a cabinet 3, a cabinet 2, a cabinet 1, a drawer 5, a drawer 4, a drawer 3, a drawer 2, a drawer 1, a dresser 1, a garbagecan 1, a safe 1, a shelf 12, a shelf 11, a shelf 10, a shelf 9, a shelf 8, a shelf 7, a shelf 6, a shelf 5, a shelf 4, a shelf 3, a shelf 2, a shelf 1, a sidetable 1, and a sofa 1.
Your task is to: put two keychain in safe.
Your admissible actions of the current situation are: [go to armchair 1, go to cabinet 1, go to cabinet 2, go to cabinet 3, go to cabinet 4, go to drawer 1, go to drawer 2, go to drawer 3, go to drawer 4, go to drawer 5, go to dresser 1, go to garbagecan 1, go to safe 1, go to shelf 1, go to shelf 10, go to shelf 11, go to shelf 12, go to shelf 2, go to shelf 3, go to shelf 4, go to shelf 5, go to shelf 6, go to shelf 7, go to shelf 8, go to shelf 9, go to sidetable 1, go to sofa 1, help, inventory, look].
...
output_text = generate_turn_unconstrained(PROMPT_TEXT)
