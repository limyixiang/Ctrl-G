...
def convert_jsonl(source: Path, destination: Path, *, indent: int | None = 2) -> tuple[int, int]:
        ...
        with source.open("r", encoding="utf-8-sig") as input_file, tempfile.NamedTemporaryFile(
            ...
            for line_number, line in enumerate(input_file, start=1):
                ...
                if indent is not None:
                    item_prefix = " " * indent
                    rendered = item_prefix + rendered.replace(
                        "\n", f"\n{item_prefix}"
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    formatting.add_argument(
        ...
        type=int,
        default=2,
        help="pretty-print with this indentation level (default: 2)",
    )
    formatting.add_argument(
        "--compact",
        action="store_true",
        help="write compact JSON with one record per line",
    )
...
def main() -> None:
    ...
    records, removed = convert_jsonl(args.input, destination, indent=indent)
    print(
