...
def load_examples(sequences_file, embeddings_file, eos_token_id):
    ...
    if os.path.isfile(embeddings_file):
        if embeddings_file.endswith('.safetensors'):
            ...
            embeddings = safe_load(embeddings_file)['embeddings'].float()
        else:
...
def Kmeans_faiss(vecs, K, max_iterations=1000, nredo=1, verbose=True):
    print(f'Initializing K-means with {K} clusters for {vecs.shape[0]} vectors...')
    ...
    print('Training K-means (this may take a while)...')
    kmeans.train(vecs)
